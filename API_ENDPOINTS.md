# API Endpoints Documentation
  ...
  - `GET /api/item-barang/{itemBarangId}/canvas-image` - Get canvas image by item barang ID
    ...
    - `status`: Filter spesifik berdasarkan status WO Planning (`draft`, `On Progress`, `completed`, `cancelled`).
    - `exclude_status`: Kecualikan status tertentu (contoh: `exclude_status=completed` untuk "selain Selesai").
  ...
  - **Filter Options**:
    - `status`: Filter berdasarkan status (nilai: `draft`, `On Progress`, `completed`, `cancelled`)
    - `exclude_status`: Mengecualikan satu status tertentu dari hasil
    - `prioritas`: Filter berdasarkan prioritas
    - `tanggal_wo`: Filter berdasarkan tanggal (format: YYYY-MM-DD)
    - **Request Examples**:
      ```
...
- **Behavior**:
  ...
  - Tahap 2 (Canvas): Untuk setiap kandidat, sistem membaca `canvas_file` JSON dan mensimulasikan ketersediaan ruang kontigu untuk `panjang × lebar` (menggunakan irisan strip vertikal dan overlap interval kosong). Hanya item yang benar-benar bisa memuat ukuran tersebut yang dikembalikan.
   - Fallback (Tanpa Canvas): Jika `canvas_file` kosong atau file tidak ditemukan/invalid, sistem akan memvalidasi langsung terhadap dimensi item (`panjang` × `lebar` pada item). Jika dimensi item cukup, item dianggap muat (fresh sheet).
