...
class PurchaseOrderController extends Controller
{
    ...
    {
        $data = PurchaseOrder::with(['purchaseOrderItems'])->onlyTrashed()->find($id);
        if (!$data) {
            return $this->errorResponse('Data tidak ditemukan', 404);
        }
    ...
    {
        $data = PurchaseOrder::with(['purchaseOrderItems'])->withTrashed()->find($id);
        if (!$data) {
            return $this->errorResponse('Data tidak ditemukan', 404);
        }
